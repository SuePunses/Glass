...
stbschedulefile = '/home/rtglass/stbController/files/schedulerState.txt' # The STB controller scheduler state file

# First check if the scheduler is on. If not, exit
sfile = open(stbschedulefile, 'r')
stateraw = sfile.read()
if stateraw == 'Disabled':
	exit()
...
stbs = []
rangesearch = re.search(r"(\d+)\s*\-\s*(\d+)", stbstring)	# Check if the input argument is actually a range (1 - 42 for example)
if rangesearch:
	if len(rangesearch.groups()) == 2:
        	start = rangesearch.group(1)
	        end = rangesearch.group(2)
	        stbs = range(int(start),int(end)+1)	# We have to add 1 "int(end)+1" to the end of the range because python considers this the number to stop at
	else:
...
def getToLive(stb):
	...
	if wakeres==0:
		...
		print(f"Wake response for {stb} is {wakeres}")
	...
	sleep(5)	# Wait for TV guide to load
	...
	#for key in chanData:
	#	channo = chanData[key]
	readyres = findLive(stb,stbip)	# Use the findLive function to get the panel on to live viewing
	if readyres is None:
