...
if len(sys.argv) < 3:
	print(f"No channel change list file provided")
	exit()

chanfile = sys.argv[2]
ccfile = testfilesdir + '/' + chanfile	# The JSON file containing channels to use
...
def createSummaryReport():
	...
	r = re.compile(r'(\d+)')	# Create a regex object which will pick out digits from the stb string
	stbrepdirs.sort(key=lambda x:int(r.search(x).group(1)))	# Sort the directory list based on numeric values ascending

	for d in stbrepdirs:
		fulldir = f"{resdir}/{d}"
		resfile = f"{fulldir}/Results.json"
		rf = open(resfile,"r")
